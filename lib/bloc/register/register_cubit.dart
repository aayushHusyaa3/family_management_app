...
class RegisterCubit extends Cubit<RegisterState> {
  ...
  Future<void> updateRole({required String role}) async {
    emit(state.copyWith(status: RegisterStatus.updating));

    try {
      await firestore.collection('users').doc(state.uid).update({"role": role});
      final doc = await FirebaseFirestore.instance
          .collection("users")
          .doc(state.uid)
          .get();
      final docRole = await doc.data()?['role'] as String?;
      await SecureStorage.save(key: 'role', data: docRole!);
      emit(state.copyWith(status: RegisterStatus.updated));
    } on FirebaseAuthException catch (exe) {
      final errorMessage = FirebaseAuthErrorHandler.getMessage(exe);
      emit(
        state.copyWith(
          status: RegisterStatus.updatingFailure,
          errorMsg: errorMessage,
        ),
      );
    }
